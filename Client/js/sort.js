sortListener.addEventListener('change', (event)=> {
    
    event.preventDefault();
    const formData = new FormData(sortSelect);
    const sortItem = formData.get('sorting');
    const attempt = {sortItem};
    ...
    async function getData(){
        const response = await fetch(allData_URL, options1);
